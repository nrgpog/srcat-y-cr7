...
import { FiShield, FiCheck, FiX, FiAlertCircle, FiCopy, FiSettings, FiGlobe, FiRefreshCw } from 'react-icons/fi';
...
declare global {
  interface Window {
    ipvanishCheckerEventSource?: EventSource;
  }
}

export default function IPVanishChecker() {
  ...
  useEffect(() => {
    return () => {
      if (window.ipvanishCheckerEventSource) {
        window.ipvanishCheckerEventSource.close();
      }
    };
  }, []);
  ...
  const processChunk = async (accounts: string[], chunkInfo: ChunkInfo) => {
    try {
      ...
      const response = await fetch('/api/ipvanish/check', {
        method: 'POST',
        headers: {
  ...
  const checkAccounts = async () => {
    ...
    try {
      ...
      const initialData = encrypt(JSON.stringify(accountList));
      const initialResponse = await fetch('/api/ipvanish/check', {
        method: 'POST',
        headers: {
          'Content-Type': 'text/plain',
        },
      ...
      for (const chunkInfo of chunks) {
        ...
        const chunkAccounts = accountList.slice(chunkInfo.startIndex, chunkInfo.endIndex);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      >
        ...
        >
          <FiShield className={`w-4 h-4 ${isChecking ? 'animate-pulse' : ''}`} />
